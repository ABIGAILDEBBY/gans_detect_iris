...
def segment_iris(face_crop, eye_mask, radius_min_para, radius_max_para):
    ...
    cv2.circle(img_ori_copy, (cx_glob, cy_glob),
               radius_glob, (255, 255, 255), 1)  # img

    iris_mask = np.logical_and(iris_mask, eye_mask)

    for i in range(img_copy.shape[0]):
        for j in range(img_copy.shape[1]):
            if not iris_mask[i][j]:
    ...
    return img_ori_copy, img_copy, iris_mask, (cx_glob, cy_glob), radius_glob, (
        eye_cx, eye_cy), highlights_global, num_refl, valid
