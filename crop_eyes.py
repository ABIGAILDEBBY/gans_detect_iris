...
def eye_detection(data, predictor_path):
    ...
    eyes_list, new_eyes_position_list, double_eye_list, double_eye_position_difference_list = crop_eye(
    ...
    for i in range(36, 47):
        ...
        cv2.circle(
            img=img, center=(
                x, y), radius=1, color=(
                255, 0, 0), thickness=-1)
        # cv2.imwrite(f'{data}',img)
    return left_eye_image, right_eye_image, new_eyes_position_list, len(
        dets), double_eye_list, double_eye_position_difference_list
