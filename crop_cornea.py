...
def cornea_convex_hull(left_eye_img, right_eye_img, new_eyes_position_list):
    ...
    """
    left_cornea = np.zeros(
        (left_eye_img.shape[0], left_eye_img.shape[1], 3), np.uint8)
    left_cornea_matrix = np.zeros(
        (left_eye_img.shape[0], left_eye_img.shape[1]), np.uint8)
    cv2.fillConvexPoly(left_cornea, new_eyes_position_list[0], (255, 255, 255))

    right_cornea = np.zeros(
        (right_eye_img.shape[0], right_eye_img.shape[1], 3), np.uint8)
    right_cornea_matrix = np.zeros(
        (right_eye_img.shape[0], right_eye_img.shape[1]), np.uint8)
    cv2.fillConvexPoly(
        right_cornea, new_eyes_position_list[1], (255, 255, 255))
