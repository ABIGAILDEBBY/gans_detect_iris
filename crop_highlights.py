...
from skimage.filters import threshold_mean, threshold_otsu, thresholding
...
def shift(
    ...
    for shifts_lr in range(negative_lr_shift, positive_lr_shift):
        for shift_ud in range(negative_ud_shift, positive_ud_shift):
            img_left_ud_shift = shiftbits(
                img_left_matrix, shift_ud, matrix=True)
            img_left_lr_shift = np.transpose(
                shiftbits(
                    np.transpose(img_left_ud_shift),
                    shifts_lr,
                    matrix=True))
            m = np.sum(
                np.logical_and(
                    img_left_lr_shift,
                    img_right_matrix).astype(int))
...
def process_aligned_image(
        ...
        reduce=True,
        reduce_size=2,
        threshold_scale_left=1,
        threshold_scale_right=1):
    """
    ...
    iris_left_HSV = cv2.cvtColor(
        iris_left_ori_reduce_iris_color,
        cv2.COLOR_BGR2HSV)
    iris_right_HSV = cv2.cvtColor(
        iris_right_ori_reduce_iris_color,
        cv2.COLOR_BGR2HSV)
    ...
    the_left_V = filter.threshold_yen(
        np.asarray(left_color_list)[
            :, 2]) * threshold_scale_left
    the_right_V = filter.threshold_yen(
        np.asarray(right_color_list)[
            :, 2]) * threshold_scale_right

    # extract highlights.
    left_recolor = np.zeros(
        (iris_left.shape[0], iris_left.shape[1], 3), dtype=np.uint8)
    right_recolor = np.zeros(
        (iris_right.shape[0], iris_right.shape[1], 3), dtype=np.uint8)
    left_recolor_matrix = np.zeros(
        (iris_left.shape[0], iris_left.shape[1]), dtype=int)
    right_recolor_matrix = np.zeros(
        (iris_right.shape[0], iris_right.shape[1]), dtype=int)
        ...
        for j in range(left_matrix.shape[1]):
            if left_matrix[i][j] == 1:
                if iris_left_HSV[i][j][2] > the_left_V:
                    ...
                    double_eye_img_modified[i +
                                            double_eye_position_difference_list[0][1]][j +
                                                                                       double_eye_position_difference_list[0][0]] = np.asarray([0, 255, 0])
                else:
        ...
        for j in range(right_matrix.shape[1]):
            if right_matrix[i][j] == 1:
                if iris_right_HSV[i][j][2] > the_right_V:
                    ...
                    double_eye_img_modified[i +
                                            double_eye_position_difference_list[1][1]][j +
                                                                                       double_eye_position_difference_list[1][0]] = np.asarray([255, 0, 0])
                else:
    ...
    right_ori_resize = np.full(
        (max_x_axis, max_y_axis, 3), 255, dtype=np.uint8)
    left_recolor_resize = np.full(
        (max_x_axis, max_y_axis, 3), 255, dtype=np.uint8)
    right_recolor_resize = np.full(
        (max_x_axis, max_y_axis, 3), 255, dtype=np.uint8)
    ...
    # draw recolor resize
    left_matrix_ud_resize = shiftbits(
        left_matrix_resize, opt_shift[0], matrix=True)
    left_matrix_lr_resize = np.transpose(
        shiftbits(
            np.transpose(left_matrix_ud_resize),
            opt_shift[1],
            matrix=True))
